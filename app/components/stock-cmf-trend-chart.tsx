...
import type { CmfTrendData, CmfTrendPoint } from "../lib/stock-cmf-trend";

type StockCmfTrendChartProps = {
  symbol: string;
  theme?: "dark" | "clear";
};

type CmfTrendResponse = {
  data: CmfTrendData;
};

type CmfTrendCoordinate = CmfTrendPoint & {
  index: number;
  x: number;
  y: number;
  zeroY: number;
...
function formatCmf(value: number) {
...
function toSourceLabel(source: CmfTrendData["source"]) {
  return source === "yahoo" ? "Yahoo Finance" : "Nasdaq";
}

export function StockCmfTrendChart({
  symbol,
  theme = "dark",
}: StockCmfTrendChartProps) {
  const [data, setData] = useState<CmfTrendData | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadCmfTrend() {
      try {
        setError(null);
        setData(null);
        setHoveredIndex(null);

        const response = await fetch(`/api/stocks/${symbol}/cmf-trend`, {
          cache: "no-store",
        });

        if (!response.ok) {
          throw new Error("Unable to load CMF trend data.");
        }

        const json = (await response.json()) as CmfTrendResponse;
        ...
        if (!isCancelled) {
          setError("CMF trend data is temporarily unavailable.");
        }
      }
    }
  ...
  const chart = useMemo(() => {
    if (points.length === 0) {
      return {
        coordinates: [] as CmfTrendCoordinate[],
    ...
    const values = points.map((point) => point.spreadVs90d);
    const maxAbs = Math.max(
      0.05,
      Math.ceil(Math.max(...values.map(Math.abs)) * 100) / 100,
    );
    const zeroY = padding + chartHeight / 2;
    const coordinates = points.map((point, index) => {
      const x = padding + (index / Math.max(points.length - 1, 1)) * chartWidth;
      const y = zeroY - (point.spreadVs90d / maxAbs) * (chartHeight / 2);
    ...
    >
      ...
      >
        <div style={{ display: "grid", gap: 5 }}>
          ...
          >
            Recent Chaikin Money Flow compared with the 90 trading-day baseline.
          </p>
        </div>
        {data ? (
          <p
      ...
      {!error && !data ? (
        <div
          style={{
            ...
            animation: "stock-cmf-trend-skeleton 1.3s ease-in-out infinite",
          }}
        />
      ) : null}
            ...
            >
              ...
              >
                ...
                <span
                  style={{
                    color:
                      latest.spreadVs90d >= 0 ? positiveFill : negativeFill,
                    fontWeight: 700,
                  }}
                >
                  {formatCmf(latest.spreadVs90d)}
                </span>
              </p>
              <p
                style={{
              ...
              >
                7D {formatCmf(latest.cmf7d)} vs 90D{" "}
                {formatCmf(latest.cmf7dAvg90d)}
              </p>
            </div>
          ) : null}
            ...
            <div style={{ minWidth: 500, position: "relative" }}>
              {hoveredPoint ? (
                ...
                >
                  ...
                  >
                    {formatCmf(hoveredPoint.spreadVs90d)}
                  </p>
                  <p
                    style={{
                      margin: "4px 0 0",
                  ...
                  >
                    7D CMF: {formatCmf(hoveredPoint.cmf7d)}
                    <br />
                    90D Avg: {formatCmf(hoveredPoint.cmf7dAvg90d)}
                  </p>
                </div>
              ) : null}
              ...
              <svg
                ...
                height={height}
                role="img"
                aria-label={`${symbol.toUpperCase()} CMF trend chart`}
                ...
                {chart.coordinates.map((point) => {
                  ...
                  return (
                    <rect
                      ...
                      rx="3"
                      fill={
                        point.spreadVs90d >= 0 ? positiveFill : negativeFill
                      }
                      opacity={
      ...
      {!error && data && points.length <= 1 ? (
        <p style={{ margin: 0, color: isClearMode ? "#dc2626" : "#fda4af" }}>
          Not enough CMF history to draw the 7D vs 90D chart.
        </p>
      ) : null}

      <style>{`@keyframes stock-cmf-trend-skeleton { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }`}</style>
    </section>
  );
}
