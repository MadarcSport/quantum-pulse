export function createCopperMaterial2(
  baseMaterial,
  { envMap = null, envMapIntensity = 1 } = {},
) {
  ...
  material.userData.lockedHdrEnvironment = true;
  material.envMapRotation.set(0, Math.PI * -0.35, 0);
  ...
  material.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      float lockedHdrSheen = smoothstep(0.62, 1.0, copperNoise(copperUv * 0.35 + vec2(7.0, 3.0)));
      ...
      diffuseColor.rgb += lockedHdrSheen * vec3(0.045, 0.022, 0.006);
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      float lockedHdrPolish = smoothstep(0.55, 1.0, copperNoise(vViewPosition.xy * 18.0 + vec2(2.5, 9.0)));
      roughnessFactor = clamp(
        roughnessFactor + copperNoise(vViewPosition.xy * 55.0) * 0.13 - lockedHdrPolish * 0.045,
        0.2,
        0.62
      );`,
    );
  ...
  material.customProgramCacheKey = () =>
    "procedural-copper-material-hdr-locked-v1";
