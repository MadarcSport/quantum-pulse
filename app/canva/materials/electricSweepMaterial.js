...
export function createElectricSweepMaterial(baseMaterial, shaderRef) {
  ...
  material.onBeforeCompile = (shader) => {
    ...
    shader.fragmentShader = shader.fragmentShader.replace(
      ...
      float sweepCycle = fract(uSweepTime * 0.4);
      ...
      float circuitSparkle = electricHash(floor(vSweepWorldPosition.xz * 8.0 + uSweepTime * 40.0));
      float electricLight = sweepCore * 1.55 + sweepTrail * 0.38 + sweepCore * circuitSparkle * 0.55;
      vec3 electricBlue = vec3(0.0, 0.3, 0.78);
